...
irc_session_t * irc_create_session (irc_callbacks_t	* callbacks)
{
	...
	session->max_timeout=CLOCKS_PER_SEC*60*5;
...
int irc_run (irc_session_t * session)
{
	...
	if(session->max_timeout==0 || session->max_timeout>CLOCKS_PER_SEC*60*60*24*5)
		session->max_timeout=CLOCKS_PER_SEC*60*5;
	session->last_ping=clock();
	session->ping_sent=FALSE;
	...
	while ( irc_is_connected(session) )
	{
		...
		ret=select (maxfd + 1, &in_set, &out_set, 0, &tv);
		if (ret<0){
			...
			unsigned int delta;
			session->server_timeout=clock();
			delta=session->server_timeout-session->last_ping;
			if(delta>=session->max_timeout){
				if(delta<session->max_timeout+CLOCKS_PER_SEC*15){ //disconnect if no pong in 15secs
					if(!session->ping_sent){
						irc_send_raw(session,"PING");
						session->ping_sent=TRUE;
					}
				}
				else
		...
		else if(ret>0 && in_set.fd_count>0){
			session->last_ping=clock();
			session->ping_sent=FALSE;
		}
		...
		else if ( !strncmp (command, "PRIVMSG", buf_end - command) )
		{
			...
			{ 
				...
				if ( params[1][0] == 0x01 && params[1][msglen-1] == 0x01 )
				{
					...
					if ( !strncasecmp(ctcp_buf, "DCC ", sizeof("DCC ")-1) )
						libirc_dcc_request (session, prefix, ctcp_buf);
					else if ( !strncasecmp( ctcp_buf, "ACTION ", sizeof("ACTION ")-1)
...
int irc_process_select_descriptors (irc_session_t * session, fd_set *in_set, fd_set *out_set)
{
	...
	if ( FD_ISSET (session->sock, out_set) )
	{
		...
		if(session->ping_sent==FALSE){
			unsigned int delta;
			session->server_timeout=clock();
			delta=session->server_timeout-session->last_ping;
			if(delta>=(session->max_timeout*3/4)) //if no recv in timeout*3/4 initiate ping
				session->last_ping=session->server_timeout-session->max_timeout;
		}
		libirc_mutex_unlock (&session->mutex_session);
	}
